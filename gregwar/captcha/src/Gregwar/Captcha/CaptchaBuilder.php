...
class CaptchaBuilder implements CaptchaBuilderInterface
{
	 ...
	 */
	protected function writePhrase($image, $phrase, $font, $width, $height, $fontRatio)
	{
	 ...
	 * Generate the image
	 */
	public function build($width = 150, $height = 40, $font = null, $fingerprint = null, $fontRatio = 0.8)
	{
		if (null !== $fingerprint) {
		...
		// Write CAPTCHA text
		$color = $this->writePhrase($image, $this->phrase, $font, $width, $height, $fontRatio);
		// Apply effects
		if (! $this->ignoreAllEffects) {
			$square  = $width * $height;
			$effects = $this->rand($square / 3000, $square / 2000);
